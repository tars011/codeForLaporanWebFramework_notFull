...
<div class="form-container">
    <div class="form-content">
        ...
        <div class="form-body">
            <form action="{{ route('item_ins.update', $itemIn->id) }}" method="post" id="inputForm">
                @method('PUT')
                @csrf
                <label for="item_id">Pilih Barang</label>
                <select id="item_id" name="item_id" class="form-control select2" readonly>
                    @foreach ($items as $item)
                    <option value="{{ $item->id }}" {{ $itemIn->item_id == $item->id ? 'selected' : '' }}>{{ $item->code.' - '.$item->name }}</option>
                    @endforeach
                </select>
                <label for="stock">Jumlah</label>
                <input type="number" id="stock" name="stock" min="1" value="{{ $itemIn->stock }}" placeholder="Contoh: 100" required>
                <label for="date">Tanggal Masuk</label>
                <input type="date" id="date" name="date" value="{{ $itemIn->date }}" required>
            </form>
        </div>
